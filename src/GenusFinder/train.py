...
def learn_curve(type_species: str, t: Tree):
    ...
    X = [n.get_distance(n_loop.name) for n_loop in n_iter.iter_leaves() if n_loop.name != ""]
    print(X)
    X = np.array(X).reshape(-1, 1)
    X *= 50
    y = [int(n_loop.name[0] == genus_name) for n_loop in n_iter.iter_leaves() if n_loop.name != ""]
    print(y)
